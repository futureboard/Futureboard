...
use cef::{
    App, CefString, CefStringUtf16, ImplApp, ImplRequest, ImplResourceHandler, ImplResponse,
    ImplSchemeHandlerFactory, ImplSchemeRegistrar, ResourceHandler, SchemeHandlerFactory,
    SchemeOptions, WrapApp, WrapResourceHandler, WrapSchemeHandlerFactory, wrap_app,
...
wrap_app! {
    ...
    impl App {
        fn on_register_custom_schemes(&self, registrar: Option<&mut cef::SchemeRegistrar>) {
            ...
            // lets the document fetch its own sibling assets.
...
wrap_scheme_handler_factory! {
    ...
    impl SchemeHandlerFactory {
        fn create(
            ...
            let url = request.map(|request| CefStringUtf16::from(&request.url()).to_string())?;
        ...
        fn response_headers(
            ...
            match self.asset {
                Some(asset) => {
                    ...
                    response.set_mime_type(Some(&CefString::from(asset.mime_type)));
...
/// plugin ids it knows.
pub fn register_plugin_scheme_factory(resolver: SchemeResolver) -> Result<(), SchemeError> {
    let mut factory = PluginSchemeFactory::new(resolver);
