...
pub fn locate_cef_dist(workspace_root: &Path) -> Option<PathBuf> {
    let dist = workspace_root.join("build").join("cef");
    if dist.join(RELEASE_DIR).is_dir() {
        Some(dist)
    } else {
        None
    }
...
pub fn stage_cef(staging_dir: &Path, dist_dir: &Path, triple: &str) -> Result<CefStageReport> {
    ...
    let release = dist_dir.join(RELEASE_DIR);
    ...
    let resources = dist_dir.join(RESOURCES_DIR);
    if resources.is_dir() {
...
mod tests {
    ...
    fn locate_returns_none_without_release() {
        ...
        assert!(locate_cef_dist(temp.path()).is_none());
    }
    ...
    fn required_files_track_platform_library() {
        assert!(required_runtime_files("x86_64-pc-windows-msvc")
            .contains(&"libcef.dll".to_string()));
        assert!(required_runtime_files("x86_64-unknown-linux-gnu")
            .contains(&"libcef.so".to_string()));
        assert!(required_runtime_files("aarch64-apple-darwin")
            .contains(&"libcef.dylib".to_string()));
    }
}
